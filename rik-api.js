...
async function teeLihtandmeteParing(otsing) {
    ...
    // Kontrollime, kas kredentsiaalid on olemas
    if (!process.env.RIK_USERNAME || !process.env.RIK_PASSWORD) {
        console.error('RIK API kredentsiaalid on puudu! Kontrolli .env faili.');
        return null;
    }
    ...
    try {
        ...
        if (error.response) {
            console.error('Serveri vastus:', error.response.data);
            console.error('Staatuskood:', error.response.status);
        }
        return null;
    }
}
...
async function main() {
    console.log('RIK kredentsiaalid:', {
        username: process.env.RIK_USERNAME,
        password: process.env.RIK_PASSWORD?.substring(0, 3) + '...' // Näitame ainult algust turvalisuse huvides
    });
