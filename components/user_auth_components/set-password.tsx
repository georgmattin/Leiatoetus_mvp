...
export default function SetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsSuccess(true)
      // Suuna kasutaja pärast edukat parooli seadmist minu-konto lehele
      setTimeout(() => {
        router.push('/minu-konto')
      }, 2000)
    } catch (error) {
      setError(error instanceof Error ? error.message : 'Parooli seadmine ebaõnnestus')
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <div className="space-y-6">
          ...
          {!isSuccess ? (
            ...
            <div className="flex items-center gap-2 text-green-600 text-base bg-green-50 p-4 rounded-md">
              ...
              <p>Parool on edukalt seadistatud! Suuname sind kohe edasi...</p>
            </div>
          )}
        </div>
      </CardContent>
